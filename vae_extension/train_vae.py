...
class Peptide_dataset_ext_labeled(Dataset):
    def __init__(self,df):
        self.device=Params.device
        self.data=df.reset_index()
        self.token=Params.dataset_params['token']
        self.max_len=Params.dataset_params['max_seq_len']
        self.condition_length=Params.dataset_params['condition_length'] 
...
def calculate_loss(model,dataset,cur_step,is_train):
    ...
    # print (seq_token[0:10])
    # print (seq_token.shape)
    # sys.exit(0)
    ...
    if is_train==True:
        ...
        print ('N_mismatch_shift',N_mismatch_shift)
        print ('original_seqs',original_seqs)
        print ('sampled_seqs_shift',sampled_seqs_shift)
...
def train(training_params,model_params,dataset_params,device):
    ...
    if Params.resume_train == False:
        ...
        model=RNN_VAE(**MP)
        optimizer = torch.optim.Adam(model.parameters(), lr=TP['lr'])
        print ('Params.resume_model_path',Params.resume_model_path)
        ...
        # sys.exit(0)
        model.load_state_dict(torch.load(Params.resume_model_path))
        optimizer = torch.optim.Adam(model.parameters(), lr=TP['lr'])
