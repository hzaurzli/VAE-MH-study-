...
TOKEN=Params.token
UNK_IDX, PAD_IDX, START_IDX, EOS_IDX=TOKEN['unk'],TOKEN['pad'],TOKEN['start'],TOKEN['eos']
...
def anneal(it):
    start_val,end_val=Params.loss_weights['beta']['val']
    start_iter,end_iter=Params.loss_weights['beta']['iter']
...
def wae_mmd_gaussianprior(z, method='full_kernel'):
    ...
    wae_mmd = Params.wae_mmd
    if method == 'full_kernel':
        mmd_kwargs = {'sigma': wae_mmd['sigma'], 'kernel': wae_mmd['kernel']}
        return mmd_full_kernel(z, z_prior, **mmd_kwargs)
    else:
        mmd_kwargs = {**wae_mmd}  # shallow copy, all cfg params.
